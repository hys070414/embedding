...
from evaluation import compute_eosk, compute_retrieval_recall, compute_intra_inter_separation, project_to_d
...
def run_experiment(token_attn_layer=None, chunk_attn_layer=None):
    ...
    # 存储结果矩阵： (t_idx, c_idx, metric)   metric: 0=EOSk,1=Recall@k,2=Separation,3=Silhouette
    results = np.full((len(t_names), len(c_names), 4), np.nan)
